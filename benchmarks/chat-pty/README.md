...
| `cli-ink-cold-start` | Production `roll chat` process through the first interactive Ink prompt | Same isolated config; after the app renders, sends one probe key, requires `› k`, clears it, makes no model request, and exits through bounded `/exit` cleanup |
