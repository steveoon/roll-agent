...
| `cli-ink-cold-start` | Production `roll chat` process through the first interactive Ink prompt | Same isolated config; no user message/model request; must render `›`, must not enter server mode, and must exit through bounded `/exit` cleanup |
