...
聊天和 Runtime 初始化只读 scheduler 账本（可读 v5/v6），只给**当前会话库**中有准确关联的
...
在 `roll chat` 里直接说「帮我每隔 30 分钟检查一下未读消息」，模型会调用内建的 `roll__schedule_create` 工具登记任务：

- 创建前会弹出确认，展示完整参数（名称、频率、工作目录、任务内容、单次时长上限、首次运行时间）与当前调度服务状态；批准一次只授权这一个任务。
- 相同定义（内容 + 目录 + 频率 + 时长上限）的 active 任务已存在时不会重复创建，工具会返回既有任务。
