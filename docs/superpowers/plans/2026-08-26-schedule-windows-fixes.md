...
- W15 追加：service install 把实际 data-dir / concurrency 以 `installing → installed` 两阶段写进用户目录下的稳定 metadata，install/uninstall 共用跨 data-dir 管理锁；partial install 会 Disable/Stop，恢复路径在 Stop + daemon fence 后重检 DB。Windows uninstall 先禁用 Scheduled Task，再 `/End`，随后持有 daemon lifecycle lock，按账本里唯一的 daemon generation 清理（inline 不受影响）。runtime 在单事务内撤销 claimed token 并返回 running ownership 快照，running exec 用 `taskkill /T /F` + 启动身份确认后按原 token 条件终态；权威 DB 缺失、空文件或 schema 不符时拒绝创建/修改。metadata 异常、manual daemon 存活或任一 exec 收尾无法确认时保留 disabled task 与 running 单例，不执行 `/Delete`
- W15 反驳者补充：POSIX 无 metadata 直接安装；installed + 无账本文件直接 `/Delete`；设置未变的 re-install 不卸载；macOS `stop()` 等待 launchd 卸载；exec 中断不写账本（`interrupted`）；Windows uninstall 幂等；lock busy / metadata 缺失文案；`requireExistingDatabase` RW 校验；cleanup 对 retry 交接与重新接管分别处理；第二轮反驳者：refresh 不经 safely 包装也不改 metadata、macOS 无 metadata 先 bootout、disabled 任务先 retire、决策抽成 `service-plan.ts` 纯函数并测全矩阵、POSIX uninstall 不依赖配置、data-dir 不存在跳过 fence、无效账本恢复提示、busy_timeout 持锁测试、schemaVersion 字面量 fixture
- 未做（记入 spec 待拍板）：W7 登录黑窗（待真机评估 S4U）、W8 后代探测
