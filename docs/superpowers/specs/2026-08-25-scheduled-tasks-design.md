...
- 待拍板（产品取舍，未改）：`setScheduleStatus` 恢复时不重算 `next_run_at`，过期后再 resume 的任务会带着回溯的 `scheduled_for` 触发（终审 reviewer 指出，是唯一能把新孤儿排到既有 dead 行前面的杠杆，只增加有界延迟）；内建 bash 工具的命令在自己的会话里运行（POSIX `detached`，`runtime/src/bash/profile.ts`），不在 exec 进程组内，进程组级探活与整组终止都覆盖不到它——重试可能与上一轮遗留的 shell 命令并行（候选修法：exec 装 SIGTERM handler 中止当前 turn 让 bash 工具自己 killTree；或调度器下让 shell 留在 exec 进程组）；Windows exec detached 后不再由 daemon 的 libuv job object 连带结束，`service stop` / `schtasks /End` 是否仍能结束它取决于 Task Scheduler 自身的 job object，需真机确认（候选补偿：stop 时对本 daemon 的 running 行走 `terminateExecutor`）；authority digest 只覆盖 `runtime.approval` / `runtime.shell`，不覆盖 Agent / tool metadata；scheduled run 创建的 Thread 不随账本清理
