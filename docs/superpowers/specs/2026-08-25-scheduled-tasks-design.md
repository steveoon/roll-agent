...
- 第七轮反驳者补充（已修）：`claimDue` 每个事务最多探活 `maxLivenessProbesPerClaim`（默认 1）个过期 running 行，其余续租等下一轮，避免 Windows 上 N×8 s 探活撑破 15 s `busy_timeout`；inline 的信号转发在 exec 退出后封口（不再向已死进程发 kill 而误判 hold），重复 Ctrl+C 升级 SIGKILL，`tree-terminated` 结果不被之后的 failed 覆盖；Windows 关闭控制台（SIGHUP）触发紧急停止跳过 grace；`whoami` SID 只取 CSV 最后一列；`cancel --kill` 在 Windows 总是提示后代不可验证
- 待拍板（产品取舍，未改）：内建 bash 工具的命令在自己的会话里运行（POSIX `detached`，`runtime/src/bash/profile.ts`），不在 exec 进程组内，进程组级探活与整组终止都覆盖不到它——重试可能与上一轮遗留的 shell 命令并行（候选修法：exec 装 SIGTERM handler 中止当前 turn 让 bash 工具自己 killTree；或调度器下让 shell 留在 exec 进程组）；Windows exec detached 后 `service stop` 不再结束在跑的 exec（候选补偿：stop 时对本 daemon 的 running 行走 `terminateExecutor`）；authority digest 只覆盖 `runtime.approval` / `runtime.shell`，不覆盖 Agent / tool metadata；scheduled run 创建的 Thread 不随账本清理
