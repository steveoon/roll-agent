...
- 第七轮反驳者补充（已修）：`claimDue` 每个事务最多探活 `maxLivenessProbesPerClaim`（默认 1）个过期 running 行，名额按 `invocations.executor_probed_at`（就地补列，持久化，`beginInvocation` 清零）升序即「最久未探活优先」分配，其余按 `livenessProbeDeferralMs`（15 s，一个轮询间隔）续租、下一轮再探，避免 Windows 上 N×8 s 探活撑破 15 s `busy_timeout`；最终 reviewer 复现过「排在前面的 alive 孤儿每轮吃掉名额」的饥饿，回归测试覆盖 alive 数达到 `claimLeaseMs / livenessProbeDeferralMs` 阈值与 store 实例重建两种情形；inline 的信号转发在 exec 退出后封口（不再向已死进程发 kill 而误判 hold），重复 Ctrl+C 升级 SIGKILL，`tree-terminated` 结果不被之后的 failed 覆盖；Windows 关闭控制台（SIGHUP）触发紧急停止跳过 grace；`whoami` SID 只取 CSV 最后一列；`cancel --kill` 在 Windows 总是提示后代不可验证
