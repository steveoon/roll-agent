...
- 第七轮（Windows 兼容评估，静态；报告 https://claude.ai/code/artifact/9447eabf-e41f-482b-82be-1ac6d1cda5fe ）：`schtasks /TR` 261 字符上限在 pnpm 全局 / Node 22.6–22.12 路径下必然超限（272 / 298），且 CLI 注册的任务默认 72 小时运行上限、无失败重启、电池供电不启动。改为 `schtasks /Create /XML` 注册：`install` 先用 `whoami` 取当前用户 SID 写入 Principal / LogonTrigger，`ExecutionTimeLimit PT0S`、`RestartOnFailure PT1M×3`、电池设置关闭，XML 以 UTF-16LE+BOM 写到 data-dir。进程身份：Windows 只信任 `%SystemRoot%`（或 `%WINDIR%`）`\System32\WindowsPowerShell\v1.0\powershell.exe` / `%ProgramFiles%\PowerShell\7\pwsh.exe`，超时 8 s，exec 读身份失败先重试一次再 terminal。daemon：`treeKillUnconfirmed` 采用最近一次 kill 结果（后续 `/T /F` 成功即清除）；win32 不再发无意义的 `taskkill /T`（无 `/F`）阶段，grace 后直接强制终止；exec 在 win32 也 `detached`；daemon 监听 SIGHUP / SIGBREAK。`cancel --kill` 在 Windows 根已退出时取消并告警「后代不可验证」。`run-now --inline` 退出后同样先看最近一次 kill 结果与执行者探活（`decideInlineExit`），树终止未确认或后代存活 / 无法探活时保留 `running`，不再无条件 `failInvocation`；inline 也监听 SIGHUP / SIGBREAK；`SpawnedInvocation.kill` 的 signal 改为必填。未改：`schtasks /End` 强杀语义（文档说明）；登录黑窗（待真机评估 S4U）；全部 Windows 行为仅有注入式单测与 PR 触发的 windows-latest job 覆盖，无真机验证
- 第七轮反驳者补充（已修）：`claimDue` 每个事务最多探活 `maxLivenessProbesPerClaim`（默认 1）个过期 running 行，名额按 `invocations.executor_probed_at`（schema v3，`addMissingColumns` 只在 `user_version < SCHEMA_VERSION` 时执行，所以必须升版本号才能给已是 v2 的账本补列——终审 reviewer 抓到的迁移缺口；`beginInvocation` 清零）升序即「最久未探活优先」分配，其余按 `livenessProbeDeferralMs`（15 s，一个轮询间隔）续租、下一轮再探，避免 Windows 上 N×8 s 探活撑破 15 s `busy_timeout`；最终 reviewer 复现过「排在前面的 alive 孤儿每轮吃掉名额」的饥饿，回归测试覆盖 alive 数达到 `claimLeaseMs / livenessProbeDeferralMs` 阈值与 store 实例重建两种情形；inline 的信号转发在 exec 退出后封口（不再向已死进程发 kill 而误判 hold），重复 Ctrl+C 升级 SIGKILL，`tree-terminated` 结果不被之后的 failed 覆盖；Windows 关闭控制台（SIGHUP）触发紧急停止跳过 grace；`whoami` SID 只取 CSV 最后一列；`cancel --kill` 在 Windows 总是提示后代不可验证
- 第八轮（W14，已修）：POSIX 内建 Bash 使用独立 session / PGID，直接终止 exec PGID 无法覆盖它。`schedule exec` 监听停止信号并把 AbortSignal 传入 JSON turn，active `AgentSession` 取消后由 Bash 自己的 `killTree` 清理独立进程组；daemon stop / maxRun、超时孤儿、`cancel --kill` 与 `run-now --inline` 统一为 POSIX SIGTERM → 有界 grace → SIGKILL，Windows 保持立即 `taskkill /T /F`。exec 退出会清除迟到升级 timer，孤儿清理 single-flight，第二次强杀前再次验证 executor 启动身份；`TOKEN_MISMATCH` 不再被新 PID/PGID 的成员误判成旧后代，避免杀掉 PID 复用后的无关进程组。命令自行再次 `setsid` / daemonize、外部 SIGKILL 与系统崩溃仍是残余边界
- 第九轮（W15，已修；Windows 行为待真机确认）：保留 exec 的 `detached` 崩溃隔离，只为显式 `schedule service uninstall` 增加补偿。service install 将解析后的 data-dir / concurrency 以 `installing → installed` 两阶段写入 `~/.roll-agent/scheduler-service.json`，install/uninstall 由独立于 data-dir 的 lifecycle lock 串行化，配置漂移时不打开错误账本；partial install 报错后会 Disable/Stop，`installing` 恢复在 Stop + daemon fence 后重检 DB，已出现则转完整清理、未出现才无账本回滚。卸载先 `/Change /Disable` 阻断 `RestartOnFailure`，再 `/End`；拿到 daemon lifecycle lock 后（manual daemon 存活则 fail-closed），按账本中唯一的 `daemon-<pid>-<uuid>` generations 清理，`inline-*` 不受影响。`prepareWorkerShutdown` 用 `BEGIN IMMEDIATE` 原子作废 claimed token 并返回 running ownership 快照，running exec 经身份校验与 `taskkill /T /F` 后用原 ownership token 条件取消。权威 DB 用 read-only probe 校验版本与 scheduler 表后才允许 RW；缺失、零字节、无关 SQLite、metadata 异常、身份 unknown、树终止失败、仍存活或 ownership 变化时不 `/Delete`，任务保留 disabled、单例保持 running
- 第九轮反驳者补充（已修）：macOS / Linux 没有 metadata 时 install 直接按配置安装（此前会抛 Windows 措辞的 fail-closed 错误）；metadata 为 installed 但 `schedules.db` 从未生成时 uninstall / 重装直接 `/Delete`（此前永久卡死）；设置未变化的重复 install 不再卸载重装（此前会终止在跑的 exec）；macOS `stop()` 在 `bootout` 后轮询 `launchctl print` 直到真正卸载（launchd 的 bootout 是异步的，立即重装会 `kickstart` 打空）；exec 被停止信号中断后不写账本（`interrupted`），由发起方决定终态，修复 `cancel --kill` 在最后一次 attempt 上把任务记成失败并暂停的 W14 回归；Windows 无 metadata 且无任务时 uninstall 幂等；lock busy 与 metadata 缺失的错误改为可读文案并附人工恢复步骤；`requireExistingDatabase` 改为 RW 打开后校验（热 journal 可回滚）、更高 schema 版本给出版本文案；cleanup 中 exec 自行把行写成 retry 视为已交接、被其他 daemon generation 重新接管才拒绝，并补齐 ownership / 重扫不变量测试；`schtasks /Create /F` 覆盖运行中任务的行为待真机确认。第二轮反驳者再补：设置未变的 refresh 只调 `controller.install()`、不改 metadata（此前失败时 `installSchedulerServiceControllerSafely` 会把本来健康的任务 Disable/End）；macOS 无 metadata 但 LaunchAgent 已加载时先 bootout 再重装（否则 daemon 带旧参数继续跑而 metadata 声称新值）；Windows 任务处于 disabled 时 install 先走 retire 完成收尾；install / uninstall 决策抽成纯函数 `planSchedulerServiceInstall` / `planSchedulerServiceUninstall`（`scheduler-host/service-plan.ts`）并覆盖全矩阵；POSIX uninstall 无 metadata 时按固定 label 卸载并清除无效 metadata；data-dir 整个不存在时跳过 daemon fence 直接删任务（不再重建目录）；账本文件无效时错误附恢复提示；`requireExistingDatabase` 的 busy_timeout 用真实子进程持锁验证；metadata schemaVersion 用字面量 fixture 钉死。未改：`terminateExecutor` 在探活已 dead 时返回 `failed` 与真实 taskkill 失败无法区分（cleanup 会拒绝并要求重试，重试即成功；候选修法是新增 already-exited 结果）
- 第十轮（先清场再落终态；设计见 `2026-08-27-scheduler-settle-before-terminal-design.md`）：成功路径此前在进程树仍存活时就写 `completed` 并释放单例，bash 工具 `&` 留下的后台进程与 MCP 孙进程无人回收且对 `cancel` / `stop` / `uninstall` 不可见。exec 现在在 `runTurn` 前（preflight）与写终态前（settle）各清场一次：树 = env 标记 `ROLL_SCHEDULE_INVOCATION` ∪ exec 自身进程组 ∪ bash / session-exec 工具登记的进程组（runtime 经 `onShellCommandSpawn` 交出子进程），SIGTERM → 2 s → SIGKILL，清不掉不写终态（新 kind `unsettled`，行保持 running 交给既有收尾），preflight 清不掉直接 `failInvocation` 不跑 turn。账本 / daemon / inline / cancel 零改动；`descendants-alive` 探针保留。实测边界：macOS 26 的 `ps -E` 只对非平台二进制展示 env，平台二进制靠进程组覆盖；`setsid` 后的纯平台二进制守护进程仍为残余；Windows 不清场。三个反驳者（内核/竞态、账本状态机、跨平台）修掉：PID 复用守卫对同号僵尸的错误豁免（真机 pid 回绕复现会误杀无关进程）、EPERM 进程不按最终快照对账导致无谓重跑、settle deadline 用墙钟、Linux 线程组首领僵尸被当死进程、session-exec 会话不登记；记录为边界：exec 组内的不可杀残留会永久保持 running（需 `cancel --abandon`）、登记组不跨 attempt、macOS `ps -E` 分不清 argv 与 env、Linux `/proc` 读取无超时
- 待拍板（产品取舍，未改）：`setScheduleStatus` 恢复时不重算 `next_run_at`，过期后再 resume 的任务会带着回溯的 `scheduled_for` 触发（终审 reviewer 指出，是唯一能把新孤儿排到既有 dead 行前面的杠杆，只增加有界延迟）；authority digest 只覆盖 `runtime.approval` / `runtime.shell`，不覆盖 Agent / tool metadata；scheduled run 创建的 Thread 不随账本清理；Windows 任务名机器全局（根文件夹 `Roll Agent Scheduler`）而 metadata 按用户存放，同一台机器多个账号各自 `install` 会互相 Disable/End 并 fail-closed——v1 不支持多账号共用 scheduler service，是否按 SID 命名任务待拍板
