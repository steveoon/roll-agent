...
| 项目 | 值 |
|---|---|
| 最新 Wire protocol | `"1.2"` |
| 兼容 Wire protocol | `"1.1"`、`"1.0"` |
...
`@roll-agent/protocol/schema`，或显式读取 `schema/1.2`、`schema/1.1`、
...
跨语言 fixture 位于 `@roll-agent/protocol/fixtures/v1.2/*`；冻结的 N-1/N-2 fixture
继续共用 `@roll-agent/protocol/fixtures/v1/*`，覆盖 1.1/1.0 compatibility suite。
...
- `InteractionId`、`RuntimeMethodForVersion<TVersion>`、
...
新代码处理 1.2/1.1 矩阵时应按协商版本调用 version-aware helper，避免把 1.2 字段泄漏到
旧 wire。
...
`RollNodeClient` 会按强制 handler 规则生成版本列表，并在 1.2 初始化后自动完成
...
|---|---|---|---|
| 已注册 `approval.request` handler | `["1.2","1.1","1.0"]` | `"1.2"` | capability ACK 后的 Server Request |
| 未注册 Server Request handler | `["1.2","1.0"]` | `"1.2"` | ACK 空集合；Runtime 不投递 Approval |
...
| 字段 | `"1.2"` | `"1.1"` | `"1.0"` |
|---|---|---|---|
| `serverRequests` | `true` | `true` | `false` |
| `serverRequestCapabilityNegotiation` | `true` | `false` | `false` |
| `approvalResolvedEvents` | `true` | `true` | `false` |
| `clientApprovalResponses` | `false` | `false` | `true` |
| `requiredServerRequestMethods` | `[]` | `["approval.request"]` | `[]` |

新 Client 向旧 Runtime 发送 `["1.2","1.1","1.0"]` 时，旧 Runtime 可选择 `"1.1"`
或 `"1.0"`；strict `initialize` params 没有增加 capability 字段。协商结果属于当前
连接，1.0/1.1 连接不能动态开启 1.2 capability。`@roll-agent/client-node` 总能广告
没有强制 handler 的 1.2；只有覆盖 `approval.request` 时才同时广告要求该 handler 的
1.1。
...
`CAPABILITY_UNAVAILABLE`。这个边界防止 1.2/1.1 Approval 绕过 Server Request 控制路径。
...
| `interactionId` | `string` | 1.2 逻辑 Interaction UUID；显式重投时保持稳定 |
...
- 1.2 `interactionId` 标识逻辑交互，显式重投会换 JSON-RPC `id` 但保持它；
...
| `client.capabilities.set` | `"1.2"` 握手扩展 | `revision`, `serverRequestMethods` | revision 与 Runtime registry 交集 |
...
activeTurn?
pendingApprovals[]
pendingInteractions[]        仅 1.2；必需字段，可为空
transcriptCompleteness      complete | legacy_snapshot
```

1.2 的 `activeTurn.status` 为 `running | cancelling | waiting-for-user`；冻结的 1.1/1.0
...
- `tool.output`
- `tool.completed`
- `approval.required`（`"1.0"` 为控制事件；`"1.2"` / `"1.1"` 仅为只读 View Event）
- `approval.resolved`（`"1.2"` / `"1.1"`）
- `turn.completed`
- `turn.cancelled`
- `turn.failed`
...
`"1.2"` 与 `"1.1"` 的 Approval 只有一条可写控制路径：Runtime 发出
...
JSON-RPC `id` 只负责当前投递的 Request/Response 关联；1.2 `interactionId` 负责逻辑
...
Interaction。断线后没有持久 Request replay/resume，未决 Server Request 会被取消，Runtime
...
| `CAPABILITY_REVISION_CONFLICT` | 1.2 capability revision 过旧，或同 revision 配不同集合 |
...
- `"1.0"` 进入 mutation 幂等缓存的七个方法是 `thread.create`、`thread.rename`、
  ...
  `approval.respond`；`"1.2"` / `"1.1"` 不使用最后一个方法。上述 mutation 都携带 UUID
  `requestId`；
