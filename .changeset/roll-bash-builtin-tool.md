...
新增内建 `roll__bash` 工具，让 roll chat 能在本机执行 shell 命令，解锁"脚本编排型 skill"（如 roll-zhipin-unread-reply）在会话中的执行。
...
**安全姿态（默认关闭，`runtime.bash.enabled`）**：

- shell 命令一律标记 `destructiveHint`，因此无论 `runtime.approval.default` 是 `guarded` 还是 `auto` 都需人工确认；只有显式配置 `runtime.approval.overrides: { "roll.bash": "auto" }` 才允许无确认执行。无策略时 fail-closed 强制确认。
