...
纯 JS 规则分类器把命令分为 known-safe / dangerous / unknown。known-safe 只读命令（`ls`/`cat`/`git status`/`grep`/`find -name` 等）自动免确认执行，dangerous（`rm -rf`、`sudo`）与 unknown 仍需人工确认。设计借鉴 codex 的白名单 + 逐命令 flag 审计（`find` 拒 `-exec`/`-delete`、`git` 只放行只读子命令并拦全局 `-c`/`--git-dir`、`sed` 仅 `-n Np`、`base64` 拒写文件等），复合命令用保守词法方案（含 `$`/反引号/重定向/子 shell 等危险元字符即降级为 unknown）替代 tree-sitter，误判方向永远偏向"更保守 → 走确认"。分类器经 config→engine→session 注入，gate/policy 零改动。关掉 `auto-approve-safe` 回归 T0 的每条确认。
...
**关键安全与生命周期**：会话进程绝不绑 turn 的 abortSignal，得以跨轮存活突破 `turnTimeout`；`exec_command` 走与 bash 同一套 gate（`--json` headless 需 `runtime.approval.overrides: { "roll.exec_command": "auto" }` 显式授权），`exec_poll` 只轮询/中断不过 gate；`AgentSession.abort()` 与 `roll chat` 退出的 finally 都会 `terminateAll()` 杀掉背景进程组，杜绝 detached 进程残留。
