...
- 新增 `roll config setup bash` 模块：交互式配置 `runtime.bash`（chat 内建 bash 工具开关、安全命令自动放行、session exec），并挂入 `roll setup` 编排为可选步骤；所有默认值保持「关」，开启必须显式选择。同步把 `runtime.bash` 段补进 config key-codec，修复 `config set/get` 对该段 kebab-case 键名不转换的问题
